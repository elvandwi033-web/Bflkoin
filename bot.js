...
function loadDB() {
  ...
  if (!fs.existsSync(DB_FILE)) {
    db = {
      ...
      pendingTopup: {},
      pendingTarik: {},
      daduPending: {},    // penyimpanan tantangan dadu yang belum dijawab
      inventory: {}       // inventory item memancing & menambang per user
    };
  } else {
    try {
  ...
  // Pastikan field baru ada di DB lama
...
// Pastikan field baru ada di user lama
...
// Kirim dana kalah ke rekening admin
...
// ============================================================
const FISH_TABLE = [
  // [nama, harga, bobot_kemunculan]
  { name: 'Ikan Busuk',     price: 0,    weight: 30 },
  { name: 'Ikan Kecil',     price: 200,  weight: 25 },
  { name: 'Ikan Mas',       price: 500,  weight: 20 },
  { name: 'Ikan Kerapu',    price: 1000, weight: 12 },
  { name: 'Ikan Tuna',      price: 2000, weight: 8  },
  { name: 'Ikan Hiu Kecil', price: 3500, weight: 4  },
  { name: 'Ikan Langka',    price: 5000, weight: 1  },
];

const MINE_TABLE = [
  { name: 'Sampah',    price: -100, weight: 35 },
  { name: 'Bottle',    price: 500,  weight: 30 },
  { name: 'Silver',    price: 600,  weight: 20 },
  { name: 'MetalScap', price: 1000, weight: 15 },
];
...
client.on('messageCreate', async (message) => {
  ...
  if (command === 'register') {
    ...
    // Tidak perlu nomor HP saat register, langsung pakai Discord ID & username
    db.users['USER_' + message.author.id] = {
      discordId: message.author.id,
      discordTag: message.author.tag,
      noHp: null,           // diisi nanti saat WD
      balance: 1000,
      registered: true,
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        { name: 'Langkah Selanjutnya', value: 'Gunakan `!help` di DM ini untuk lihat semua fitur.\nNomor DANA hanya diperlukan saat kamu ingin WD (`!tarik`).' }
      );
    return message.reply({ embeds: [embed] });
  }
  ...
  if (command === 'tip') {
    ...
    // Saldo TIDAK dipublik, hanya info tip
    const embed = new EmbedBuilder()
  ...
  if (command === 'rain' || command === 'party') {
    ...
    const embed = new EmbedBuilder()
      ...
      .setColor(C_GOLD)
      // Saldo pengirim TIDAK ditampilkan
  ...
  if (command === 'dadu') {
    ...
    // Cek jika sudah ada tantangan pending untuk target ini
    if (db.daduPending[target.id]) {
      return message.reply(target.username + ' masih punya tantangan dadu yang belum dijawab!');
    }
  ...
  if (command === 'acc') {
    ...
    if (diceA > diceB) {
      ...
      user.balance -= bet;
      sendToAdmin(db, 0); // tidak ada yang ke admin jika ada pemenang jelas
      ...
      // Seri: taruhan kedua belah pihak masuk ke admin
      challenger.balance -= bet;
      user.balance -= bet;
  ...
  if (command === 'mancing') {
    ...
    user.balance -= cost;
    sendToAdmin(db, cost); // biaya mancing masuk ke admin
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription('Kamu berhasil mendapat **' + fish.name + '**!')
      .addFields(
        ...
        { name: 'Cara Jual', value: 'Ketik `!jual ikan` untuk jual semua ikan', inline: false }
      )
      .setFooter({ text: 'Saldo sekarang: ' + user.balance.toLocaleString('id-ID') + ' BFL' });
    return message.reply({ embeds: [embed] });
  }
  ...
  if (command === 'tambang') {
    ...
    user.balance -= cost;
    sendToAdmin(db, cost); // biaya tambang masuk ke admin

    const material = weightedRandom(MINE_TABLE);
    const inv      = getInventory(db, message.author.id);

    if (material.price < 0) {
      // Sampah: langsung kurangi saldo tambahan, tidak masuk inventori
      user.balance += material.price; // -100
      if (user.balance < 0) user.balance = 0;
      saveDB(db);
      const embed = new EmbedBuilder()
        .setTitle('⛏️ Hasil Menambang')
        .setColor(C_RED)
        .setDescription('Kamu menemukan **Sampah**! Rugi 100 BFL tambahan.')
        .setFooter({ text: 'Saldo sekarang: ' + user.balance.toLocaleString('id-ID') + ' BFL' });
      return message.reply({ embeds: [embed] });
    }
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription('Kamu menemukan **' + material.name + '**!')
      .addFields(
        ...
        { name: 'Cara Jual', value: 'Ketik `!jual tambang` untuk jual semua material', inline: false }
      )
      .setFooter({ text: 'Saldo sekarang: ' + user.balance.toLocaleString('id-ID') + ' BFL' });
    return message.reply({ embeds: [embed] });
  }
  ...
  if (command === 'inventori' || command === 'inventory') {
    ...
    const embed = new EmbedBuilder()
      ...
      .setColor(C_BLUE)
      .addFields(
        { name: '🐟 Ikan (' + ikan.length + ' item)', value: ikanList.slice(0, 400), inline: false },
        { name: '⛏️ Material (' + tambang.length + ' item)', value: tambangList.slice(0, 400), inline: false }
      );
    return message.reply({ embeds: [embed] });
  }

  // ======================== !box (daily box, DM only) ========================
  if (command === 'box') {
  ...
  // ======================== !beli vip ========================
  if (command === 'beli' && args[0]?.toLowerCase() === 'vip') {
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        { name: 'Keuntungan', value: 'Akses ke `!boxpremium` dengan hadiah **3.000-7.000 BFL**\nWin Rate 30%, kalah 70% (tidak dapat apa-apa)', inline: false },
        { name: 'Cara Pakai', value: 'Ketik `!boxpremium` di DM bot ini', inline: false }
      );
    return message.reply({ embeds: [embed] });
  }
  ...
  if (command === 'boxpremium') {
    ...
    if (isWin) {
      const reward = Math.floor(Math.random() * 4001) + 3000; // 3000-7000
      user.balance += reward;
      saveDB(db);
      ...
      const embed = new EmbedBuilder()
        ...
        .setFooter({ text: 'VIP Dewa Kera tetap aktif selama tidak dinonaktifkan' });
      return message.reply({ embeds: [embed] });
    }
  }
  ...
  if (command === 'tarik') {
    ...
    // Cek jika nomor sudah dipakai akun lain
    ...
    // Simpan/update nomor DANA ke profil user
    user.noHp = noHp;
    user.balance -= amount;
