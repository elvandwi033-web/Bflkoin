...
client.on('messageCreate', async (message) => {
  ...
  if (command === 'help') {
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        { name: '🎰 SLOT (BFL/IDR)', value: '`!slot <taruhan>` - BFL (WR 40:60)\n`!slotidr <taruhan>` - IDR (WR 30:70)' },
        { name: '🎰 BONANZA SLOT', value: '`!bonanza <taruhan>` - Min 20.000 | Max 100.000 BFL\nWR 40:60 | Hadiah maks x50 (Langsung main, no beli fitur!)' },
  ...
  if (command === 'acc') {
    ...
    if (diceA > diceB) {
      ...
      else { challenger.balance += bet; user.balance -= bet; taxAmount = applyTax(db, challenger, bet); addDailyLoss(user, bet); }
      ...
      else { user.balance += bet; challenger.balance -= bet; taxAmount = applyTax(db, user, bet); addDailyLoss(challenger, bet); }
  ...
  if (command === 'slot') {
    ...
    // #6: VIP WR +20%, base WR 40:60, admin WR 85%
    const winChance = message.author.id === ADMIN_ID ? 0.85 : (user.isVIP ? 0.60 : 0.40);
    const symbols = ['🍒', '🍋', '🍊', '🍇', '⭐', '💎', '7️⃣'];
    const spin = [
  ...
  if (command === 'slotidr') {
    ...
    // #12: WR 30:70, VIP +20% = 50:50, admin 85%
    const winChance = message.author.id === ADMIN_ID ? 0.85 : (user.isVIP ? 0.50 : 0.30);
    const symbols = ['🍒', '🍋', '🍊', '🍇', '⭐', '💎', '7️⃣'];
    const spin = [
      symbols[Math.floor(Math.random() * symbols.length)],
      symbols[Math.floor(Math.random() * symbols.length)],
    ...
    const embed = new EmbedBuilder()
      ...
      .setColor(multiplier > 0 ? C_GREEN : C_RED)
      .setDescription(line + '\n\n⚠️ WR 30:70 (IDR Mode)' + (user.isVIP ? ' | VIP: WR 50:50' : '') + (message.author.id === ADMIN_ID ? ' | ADMIN: WR 85%' : ''))
      .addFields(
  ...
  // ======================== !bonanza (WR 40:60, min 20k, maks 100k, hadiah maks x50) ========================
  if (command === 'bonanza') {
    if (!user) return message.reply('Belum terdaftar!');
    if (isStarving(user)) return message.reply('❌ Kamu kehabisan makan! Beli dulu dengan `!beli makan`');
    ensureUserFields(user);
    ...
    const bonanzaWinChance = isAdminPlayer ? 0.85 : (user.isVIP ? 0.60 : 0.40);
    ...
    if (Math.random() < bonanzaWinChance) {
      // Hitung matches
      const counts = {};
      ...
      for (const [sym, count] of Object.entries(counts)) {
        if (count >= 3) {
          const baseMulti = count === 3 ? 2 : count === 4 ? 5 : count === 5 ? 12 : 25;
          const symMulti = sym === '💎' ? 2 : sym === '⭐' ? 1.5 : 1;
      ...
      // Jika tidak ada 3-match, beri multiplier kecil agar tetap menang
      if (totalMultiplier === 0) totalMultiplier = 1.5;

      // VIP bonus
      if (user.isVIP) totalMultiplier = Math.floor(totalMultiplier * 1.2);

      // Hard cap x50
      if (totalMultiplier > 50) totalMultiplier = 50;
    }
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription('**[ ' + gridDisplay + ' ]**\n' + (cascades > 0 ? '✨ ' + cascades + ' Cascade Win!' : totalMultiplier > 0 ? '✅ Menang!' : '😞 Tidak ada kombinasi'))
      .addFields(
        { name: 'Taruhan', value: bet.toLocaleString('id-ID') + ' BFL', inline: true },
        ...
        { name: 'Info', value: 'WR 40:60 | Min 20k | Max 100k | Hadiah maks x50', inline: false }
      );
  ...
  if (command === 'mancing') {
    ...
    // #4: WR 50:50, admin 85%
    const isWin = Math.random() < (message.author.id === ADMIN_ID ? 0.85 : (user.isVIP ? 0.70 : 0.50));
    user.balance -= cost;
    sendToAdmin(db, cost);
  ...
  if (command === 'main' && args[0]?.toLowerCase() === 'casino') {
    ...
    for (const player of db.partyCasino.players) {
      ...
      let multiplier = 0;
      const winChance = player.id === ADMIN_ID ? 0.85 : (pUser.isVIP ? 0.70 : 0.50);
      if (Math.random() < winChance) {
        if (spin[0] === spin[1] && spin[1] === spin[2]) {
          if (spin[0] === '💎') multiplier = 10;
          else if (spin[0] === '7️⃣') multiplier = 7;
  ...
  if (command === 'cashback') {
    ...
    return message.reply({ embeds: [
      new EmbedBuilder()
        ...
        .addFields(
          { name: '📉 Total Kalah Hari Ini', value: user.dailyLoss.toLocaleString('id-ID') + ' BFL', inline: true },
          { name: '💰 Cashback Diterima', value: '+' + cashbackAmount.toLocaleString('id-ID') + ' BFL', inline: true },
          { name: '💵 Saldo BFL Sekarang', value: user.balance.toLocaleString('id-ID') + ' BFL', inline: false }
        )
    ...
    try {
      await (await client.users.fetch(target.id)).send({ embeds: [
        new EmbedBuilder()
          ...
          .setColor(C_RED)
          .setDescription('Admin telah mengurangi **' + amount.toLocaleString('id-ID') + ' BFL** dari saldo kamu.')
          .addFields({ name: 'Saldo BFL Sekarang', value: targetUser.balance.toLocaleString('id-ID') + ' BFL' })
      ]});
    } catch (e) {}
    return message.reply('✅ Saldo BFL ' + target.username + ' dikurangi ' + amount.toLocaleString('id-ID') + ' BFL. Sisa: ' + targetUser.balance.toLocaleString('id-ID') + ' BFL');
  }
