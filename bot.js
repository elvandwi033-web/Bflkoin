...
// ============================================================
// DATABASE - MongoDB Atlas (data tidak hilang saat deploy ulang)
// ============================================================
const dbSchema = new mongoose.Schema({ _id: String, data: Object }, { _id: false });
const DBStore  = mongoose.models.DBStore || mongoose.model('DBStore', dbSchema);

mongoose.connect(process.env.MONGODB_URI)
  .then(() => console.log('✅ MongoDB terhubung!'))
  .catch(e => { console.error('❌ MongoDB gagal konek:', e.message); process.exit(1); });

async function loadDB() {
  const doc = await DBStore.findById('main').lean();
  const db  = doc ? doc.data : null;
  ...
  if (!db.pendingTopup) db.pendingTopup = {};
  if (!db.pendingTarik) db.pendingTarik = {};
...
if (!process.env.MONGODB_URI) { console.error('ERROR: MONGODB_URI belum diset!'); process.exit(1); }
