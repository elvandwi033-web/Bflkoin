...
// ============================================================
// CONFIG - diambil dari Environment Variables Railway
// ============================================================
...
if (!TOKEN) {
  console.error('ERROR: DISCORD_TOKEN belum diset di Variables Railway!');
  process.exit(1);
}
if (!ADMIN_ID) {
  console.error('ERROR: ADMIN_DISCORD_ID belum diset di Variables Railway!');
  process.exit(1);
}
...
client.once('ready', () => {
  ...
  // Set admin discordId saat pertama start
  const db = loadDB();
...
client.on('messageCreate', async (message) => {
  ...
  if (command === 'help') {
    const embed = new EmbedBuilder()
      ...
      .setColor(C_GOLD)
      .setDescription('1 BFL = 1 Rupiah | Register hanya di DM bot!')
      .addFields(
        { name: 'REGISTRASI (DM Bot)', value: '!register <no_dana>\nContoh: !register 08123456789' },
        { name: 'CEK SALDO', value: '!saldo - Cek saldo koin kamu' },
        { name: 'PROFIL', value: '!profile - Lihat profil lengkap' },
        { name: 'TIP', value: '!tip @user <jumlah> - Kirim koin ke orang lain' },
        { name: 'RAIN / PARTY', value: '!rain <jumlah> - Bagikan koin ke yang aktif' },
        { name: 'DADU 1v1', value: '!dadu @user <taruhan> - Adu dadu, terbesar menang!' },
        { name: 'SLOT MACHINE', value: '!slot <taruhan> - Main slot mesin!' },
      ...
      .setFooter({ text: 'DANA Admin: ' + DANA_ADMIN + ' | !help untuk panduan' });
    return message.reply({ embeds: [embed] });
  }

  // ======================== !register ========================
  if (command === 'register') {
    if (!isDM) return message.reply('Register hanya bisa di DM (pesan pribadi) bot ini!');
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        { name: 'Langkah Selanjutnya', value: 'Lakukan !topup di DM ini untuk isi saldo.' }
      );
    return message.reply({ embeds: [embed] });
  }
  ...
  // ======================== !saldo ========================
  if (command === 'saldo') {
    if (!user) return message.reply('Belum terdaftar! Ketik !register <no_dana> di DM bot.');
    const embed = new EmbedBuilder()
      .setTitle('Saldo BFL Coin')
      .setColor(C_GOLD)
  ...
  if (command === 'tip') {
    ...
    if (!user) return message.reply('Belum terdaftar! Daftar dengan !register di DM bot.');
  ...
  if (command === 'dadu') {
    ...
    if (user.balance < bet) return message.reply('Saldo tidak cukup! Saldo: ' + user.balance.toLocaleString('id-ID') + ' BFL');
  ...
  if (command === 'topup') {
    ...
    if (!user) return message.reply('Belum terdaftar! Ketik !register <no_dana> di sini dulu.');
    ...
    const embed = new EmbedBuilder()
      ...
      .setColor(C_BLUE)
      .setDescription('Transfer Rp' + amount.toLocaleString('id-ID') + ' ke DANA berikut ini:')
      .addFields(
  ...
  if (command === 'tarik') {
    ...
    try {
      ...
      console.log('Gagal kirim notif ke admin:', e.message);
    }
  ...
  if (command === 'topupok') {
    ...
    try {
      ...
      const embed = new EmbedBuilder()
        ...
        .setColor(C_GREEN)
        .setDescription('Topup ' + amount.toLocaleString('id-ID') + ' BFL berhasil dikreditkan ke akunmu!')
        .addFields({ name: 'Saldo Baru', value: targetUser.balance.toLocaleString('id-ID') + ' BFL' });
      await u.send({ embeds: [embed] });
    } catch (e) {
      console.log('Gagal DM user:', e.message);
    }
  ...
  if (command === 'reject') {
    ...
    try {
      const u = await client.users.fetch(targetId);
      await u.send('Request tarik Rp' + pending.amount.toLocaleString('id-ID') + ' ditolak oleh admin. Saldo telah dikembalikan.');
    } catch (e) {
      console.log('Gagal DM user:', e.message);
    }
