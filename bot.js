...
// ============================================================
// CONSTANTS (harus di atas loadDB)
// ============================================================
...
function loadDB() {
  ...
  // ============================================================
  // SELALU sync akun admin dengan ADMIN_ID dari env variable
  // Ini fix masalah: nomor admin terdaftar di akun lain / ID salah
  // ============================================================
  if (process.env.ADMIN_DISCORD_ID) {
    // Hapus entry lama yang mungkin pakai ID salah atau ADMIN_PLACEHOLDER
    for (const key of Object.keys(db.users)) {
      const u = db.users[key];
      if (u.noHp === DANA_ADMIN && u.discordId !== process.env.ADMIN_DISCORD_ID) {
        console.log('[DB] Menghapus entry admin lama: ' + key + ' (discordId=' + u.discordId + ')');
        delete db.users[key];
      }
    }
    // Pastikan entry admin selalu ada dan benar
    if (!db.users['ADMIN_' + DANA_ADMIN] || db.users['ADMIN_' + DANA_ADMIN].discordId !== process.env.ADMIN_DISCORD_ID) {
      ...
      console.log('[DB] Admin entry di-sync: discordId=' + process.env.ADMIN_DISCORD_ID);
    }
...
client.once('clientReady', () => {
  ...
  console.log('Admin ID dari env: ' + ADMIN_ID);

  // ============================================================
  // FIX: Selalu sync Admin ID dari env variable setiap bot start
  // Ini mengatasi masalah database lama menyimpan ID yang salah
  // ============================================================
  const db = loadDB();
  const adminEntry = db.users['ADMIN_085640241324'];

  if (adminEntry) {
    if (adminEntry.discordId !== ADMIN_ID) {
      console.log('Mengupdate Admin ID: ' + adminEntry.discordId + ' -> ' + ADMIN_ID);
      adminEntry.discordId = ADMIN_ID;
      adminEntry.discordTag = 'Admin';
      saveDB(db);
      console.log('Admin ID berhasil diupdate!');
    } else {
      console.log('Admin ID sudah benar: ' + ADMIN_ID);
    }
  } else {
    // Admin entry tidak ada, buat baru
    db.users['ADMIN_085640241324'] = {
      discordId: ADMIN_ID,
      noHp: DANA_ADMIN,
      balance: 225000,
      registered: true,
    ...
    saveDB(db);
    console.log('Admin entry dibuat baru dengan ID: ' + ADMIN_ID);
  }
...
client.on('messageCreate', async (message) => {
  ...
  console.log('[CMD] ' + message.author.tag + ' | ' + command + ' | DM=' + isDM + ' | isAdmin=' + (message.author.id === ADMIN_ID));
  ...
  // ======================== !admincheck (debug) ========================
  // Gunakan command ini untuk cek apakah kamu dikenali sebagai admin
  if (command === 'admincheck') {
    if (message.author.id !== ADMIN_ID) {
      return message.reply('❌ Kamu BUKAN admin.\nID kamu: `' + message.author.id + '`\nAdmin ID di env: `' + ADMIN_ID + '`');
    }
    return message.reply('✅ Kamu adalah ADMIN!\nID: `' + message.author.id + '`');
  }
});
