...
client.on('messageCreate', async (message) => {
  ...
  if (command === 'slotidr') {
    ...
    const line = '[ ' + spin.join(' | ') + ' ]';
    const hasilText = multiplier > 0 ? '🎉 MENANG x' + multiplier + '!' : '😞 Kalah';
  ...
  if (command === 'bonanza') {
    ...
    const winLabel = chosenMult === 50 ? '🔥 JACKPOT x50!!' : chosenMult >= 5 ? '💥 SUPER WIN x' + chosenMult + '!' : chosenMult > 0 ? '✅ Menang x' + chosenMult : '😞 Tidak ada kombinasi';
  ...
  if (command === 'main' && args[0]?.toLowerCase() === 'casino') {
    ...
    // Kumpulkan saldo dari semua pemain ke pool
    let pool = 0;
    const participated = [];
    const skipped = [];
    ...
    for (const player of db.partyCasino.players) {
      ...
      pUser.balance -= bet;
      pool += bet;
      // Kekalahan masuk admin
      sendToAdmin(db, bet);
      consumeHunger(pUser, 'slot');
      participated.push({ player, pUser });
    }
    ...
    // Pilih satu pemenang secara acak dari yang ikut
    const winnerEntry = participated[Math.floor(Math.random() * participated.length)];
    winnerEntry.pUser.balance += pool;
    // Kembalikan dari admin (admin tadi menerima semua bet, kurangi pool kemenangan)
    ...
    const playerLines = participated.map(e =>
      e.player.id === winnerEntry.player.id
        ? '🏆 <@' + e.player.id + '>: **MENANG!**'
        : '❌ <@' + e.player.id + '>: Kalah'
    ).join('\n');
    ...
    const embed = new EmbedBuilder()
      .setTitle('🎮 Party Casino - Selesai!')
      .setColor(C_GOLD)
      .setDescription('**Total Pool:** ' + pool.toLocaleString('id-ID') + ' BFL\n\n' + playerLines + skippedLines + '\n\n🎉 Pemenang: <@' + winnerEntry.player.id + '> membawa semua!')
      .addFields({ name: 'Sesi Baru', value: '`!party casino` untuk mulai lagi' });
    db.partyCasino = null;
    saveDB(db);
    return message.reply({ embeds: [embed] });
  }
